...
gulp.task('buildexpandedcss', function() {
  ...
  .pipe(autoprefixer(['last 15 versions', 'ie >= 7', 'Firefox < 20']))
  .pipe(gulp.dest('dist/source-files/ramio-css-files'));
});
...
gulp.task('buildexpandedlibs', function() {
  ...
  .pipe(concat('libs.js'))
  .pipe(gulp.dest('dist/source-files/ramio-js-files'));
});
...
gulp.task('build', ['clean', 'sass', 'buildhtml', 'pics', 'libs', 'buildexpandedcss', 'buildexpandedlibs'], function() {
  ...
  // JSON Files
  var buildAssets = gulp.src('app/assets/**/*').pipe(gulp.dest('dist/source-files/ramio-json-files'));
  ...
  // Malihu Plugin Files
  var buildMalihu = gulp.src([
    'bower_components/malihu-custom-scrollbar-plugin/jquery.mCustomScrollbar.js',
    'bower_components/malihu-custom-scrollbar-plugin/jquery.mCustomScrollbar.css'
  ])
  .pipe(gulp.dest('dist/source-files/malihu-custom-scrollbar-plugin'));
  ...
  // Bootstrap Grid Unminified
  var buildBootstrap = gulp.src('bower_components/bootstrap/bootstrap.css').pipe(gulp.dest('dist/source-files/bootstrap'));
